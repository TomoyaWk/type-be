...
async function seedingDB() {
    //seeding
    const resp = await index_1.default.request(`http://localhost:3000/api/signup`, {
        method: "POST",
...
async function login() {
    const auth = await index_1.default.request(`http://localhost:3000/api/login`, {
        method: "POST",
...
describe("TodoAPI", () => {
    describe("GET 一覧取得", () => {
        it("正常系 一覧", async () => {
            ...
            const res = await index_1.default.request("http://localhost:3000/api/todos", {
                method: "GET",
        ...
        describe("GET １件取得", () => {
            it("正常系", async () => {
                ...
                await seedingDB();
                const token = await login();
                const recent = await prisma_1.default.task.findFirst({ orderBy: { createdAt: "asc" } });
                const res = await index_1.default.request(`http://localhost:3000/api/todos/${recent?.id}`, {
                    method: "GET",
                    headers: { "Authorization": `Bearer ${token}` }
                });
                expect(res.status).toBe(200);
        ...
        describe("POST 追加", () => {
            it("正常系", async () => {
                ...
                const created = await prisma_1.default.task.findFirst({ where: {
                        title: newTask.title
                    } });
        ...
        describe("DELETE 削除", () => {
            it("正常系", async () => {
                ...
                expect(res.status).toBe(200);
                const deleted = await index_1.default.request(`http://localhost:3000/api/todos/${recent?.id}`, {
                    method: "GET",
                    headers: { "Authorization": `Bearer ${token}` }
                });
                console.log(deleted);
